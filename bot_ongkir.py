...
def search_destination(keyword: str) -> list:
    try:
        # Endpoint resmi: domestic-destination dengan param search
        r = requests.get(
            f"{COST_BASE}/destination/domestic-destination",
            headers={"key": KOMERCE_COST_KEY},
            params={"search": keyword, "limit": 10, "offset": 0},
            timeout=10
        )
        data = r.json()
        logger.info(f"search_destination [{keyword}]: {data}")
